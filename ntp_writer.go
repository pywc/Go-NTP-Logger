...
func getNewFileName() string {
	files, _ := filepath.Glob(OUTPUT_FILE_PREFIX + "*.pcap")
	newFilename := fmt.Sprintf("%s-%s.pcap", OUTPUT_FILE_PREFIX, getCurrentDate())
	...
	for _, file := range files {
		if newFilename == file {
			...
			newFilename = fmt.Sprintf("%s-%s_%02d%02d%02d.pcap", OUTPUT_FILE_PREFIX, getCurrentDate(), hours, minutes, seconds)
		}
	}
